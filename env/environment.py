...
class Env(object):
    ...
    def __init__(self, num_of_carts=1, end_time=14 * 3600, beta=0.001, convex_alpha=0.8):
        ...
        self._previous_states = {cart.cartID: [] for (_, cart) in self._carts.items()}
        self._previous_actions_list = {cart.cartID: [0] * 2 for (_, cart) in self._carts.items()}
    ...
    def get_state(self):
        ...
        # temp = self.passenger_vector()

        # passvecnorm = np.array([0 if j==0 else 1 for j in temp])
        passvecnorm = np.array([1 if len(cart_stops.passengers) else 0 for (_, cart_stops) in sorted(self._stops.items())])
    ...
    def step(self, chosen_action=None):
        ...
        if chosen_action:
            ...
            self._previous_states[self.current_cart.cartID] = passengerVec

            self._previous_actions_list[self.current_cart.cartID].insert(0, action)
            self._previous_actions_list[self.current_cart.cartID].pop()
                ...
                if self._current_event_type != 'car_arrival_past':
                    ...
                    if action == 'agent_action':
                        ...
                        if len(self._carts_decision_times[self.current_cart.cartID]) >= 1:
                            ...
                            # previous_state = self._action_states[self.carts[cart_index].cartID][0][2:10].sum()
                            previous_state = self._previous_states[self.current_cart.cartID]
                            # previous_action = self._cstate_reward_dict[self.current_cart.cartID][-1][1]
                            previous_action = self._previous_actions_list[self.current_cart.cartID]
                            cart_occupancies = [cart.occupancy for (key, cart) in self._carts.items()]

                            delta_reward = 0  # no need to compute here
                            self._delta_reward = delta_reward

                            reward_passenger = self.current_cart.computeCost_R(ta, ta_prime)  # all rewards are the costs
                            reward_battery = self.current_cart.battery_charge_cost(charge_ta, charge_ta_prime)


                            # is_there_cart_behind = self.stops[self.current_cart.previous_stop]._occupied_by_cart

                            if sum(previous_state) + sum(passengerVec) == 0 \
                                    and previous_action == ['continue_past_next'] * 2 \
                                    and sum(cart_occupancies) == 0:

                                reward = reward_battery * 0.15
                                # reward = 1.0

                                # reward = (self._convex_alpha) * reward_passenger + \
                                #          (1.0 - self._convex_alpha) * reward_battery

                            # elif sum(previous_state) + sum(passengerVec) == 0 \
                            #         and previous_action == ['stop', 'wait']  \
                            #         and sum(cart_occupancies) == 0 and not is_there_cart_behind:
                            #
                            #     reward = -6.0

                            else:
                                reward = reward_passenger
                                # reward = (self._convex_alpha) * reward_passenger + \
                                #          (1.0 - self._convex_alpha) * reward_battery

                            # reward = (self._convex_alpha)* reward_passenger + (1.0 - self._convex_alpha)*reward_battery
                            #
                            # self._reward = reward_passenger

                            # # <---- Final Battery Reward ---- >
                            # if  self.current_cart._charge_used_cumsum < 3.6:
                            #     self._reward = self.clip(reward_passenger, min=-10, max=10)
                            #
                            # else:
                            #
                            #     self._reward = self.clip(reward_passenger + reward_battery , min=-10, max=10)
                            #
                            # # <---- Final Battery Reward ---- >

                            self._reward = self.clip(reward)

                            #
                            # # <---- running mean - std --->
                            # mean_reward, std_reward = self._func_procs['reward'].send(reward_passenger)
                            #
                            # if std_reward != 0:
                            #     self._reward = (reward_passenger - mean_reward) / std_reward
                            #
                            # else:
                            #     self._reward = reward_passenger - mean_reward
                            #
                            # self._reward = self.clip(reward_passenger)
                            #
                            #
                            # # <---- running mean - std --->
