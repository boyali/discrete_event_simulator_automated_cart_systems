...
observations = env.reset()  # prepare processes
done = False
...
# while not done:
#     action, _ = env.current_cart.sampleAction()
#     s_, r_, done = env.step(action)
#
# print(env._pass_mean_time)
#
# for (_, cart) in env.carts.items():
...
file_handler_sim = logging.FileHandler('../tests/logs/log_std_sim_N_{}.txt'.format(N))
open('../tests/logs/log_std_sim_N_{}.txt'.format(N), 'w').close()
file_handler_sim.setFormatter(formatter)
file_handler_sim.flush()

open('../tests/logs/log_std_sim_N_{}.txt'.format(N), 'w').close()
logger_sim = logging.getLogger('../tests/logs/log_std_sim_N_{}.txt'.format(N))
...
cstate_reward_pd.to_csv('../tests/logs/cars_cstate_reward_pd_collected_{}_std.csv'.format(N), sep="\t")
passenger_waiting_pd.to_csv('../tests/logs/passenger_waiting_pd_collected_{}_std.csv'.format(N), sep="\t")
sars_pd.to_csv('../tests/logs/sars_collected_{}_std.csv'.format(N), sep="\t")
agents_pd.to_csv('../tests/logs/cars_agent_stats_pd_collected_{}_std.csv'.format(N), sep="\t")
